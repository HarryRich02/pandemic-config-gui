...
# Import the graph logic from your separate file
import graph
...
class DistributionEditor(QtW.QWidget):
    ...
    def __init__(self, label_text, default_type="constant"):
        super().__init__()
        # FIX: Renamed self.layout to self.main_layout to avoid shadowing QWidget.layout()
    ...
    def _update_fields(self, dist_type):
        ...
        # Use Qt.LayoutResize mode to force a recalc
...
class CollapsibleBox(QtW.QWidget):
    ...
    It automatically resizes if the content changes size while open.
    """
    ...
    def on_pressed(self):
        ...
        # Calculate new height using the widget's layout size hint
        # self.content_area.layout() is now safe to call because we fixed the shadowing
    ...
    def on_content_resize(self):
        ...
        if self.toggle_button.isChecked():
            # Recalculate height based on new fields
...
class AccordionWidget(QtW.QWidget):
    ...
    def __init__(self):
        ...
        self.boxes = []
        self.layout.addStretch(1)  # Pushes boxes to top
    ...
    def add_item(self, title, widget):
        ...
        # Connect signal to enforce accordion behavior (one open at a time)
